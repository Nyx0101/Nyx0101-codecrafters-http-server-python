...
class HTTPRequest:
    ...
    @staticmethod
    def from_bytes(reques_bytes: bytes) -> "HTTPRequest":
        request = HTTPRequest()
        ...
        request.method, request.path, request.http_version = [b.decode() for b in line.split(b" ")]
...
def main() -> None:
    ...
    while True:
        ...
        t = threading.Thread(target=request_handler, args=(sock,))
        t.start()
...
def request_handler(sock: socket.socket) -> None:
    ...
    response_contents = [f"{request.http_version} {response_code}", *[f"{key}: {value}" for key, value in headers.items()], "", response_body]
    
    sock.send("\r\n".join(respose_contents).encode())
